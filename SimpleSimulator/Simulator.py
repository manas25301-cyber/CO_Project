...
def R_type(instruction):
    ...
    src1 = register(rs1)  #register function to call the required register(bit to names)
    src2 = register(rs2)
    dest = register(rd)

    value1=registers[src1]   #registers function for reading the value in register internally(read registers value)
...
def S_type(current_pc, instr):
    ...
    imm = B_To_D(imm1 + imm2)
    src1 = register(rs1)
    src2 = register(rs2)

    if funct3 == "010":
        addr = registers[src1] + imm
        memory[addr] = registers[src2]
...
def J_type(current_pc, instruction, i):
    ...
    if new_pc % 4 != 0:
        with open(output_file, 'a') as f:
            f.write(f"Error: Misaligned PC at address {new_pc:032b}\n")
